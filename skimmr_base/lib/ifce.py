...
class MemStoreQueryResult:
  ...
  def generate_json(self,res_type='stmt'):
    # generate a JSON representation of the result (either statements or
    # provenances depending on the input argument)

    # converting the result data into a NetworkX graph and then JSON
    G = self._get_nx_graph(res_type=res_type)
    data = json_graph.adjacency_data(G)
    # transforming the NetworkX JSON into InfoVis JSON
    infovis = []
    # building the node info list
    i = 0
    for node in data['nodes']:
      node_dict = {
        'id' : unicode(node['id']),
        'name' : unicode(node['label']),
        'data' : {'weight':unicode(node['weight'])},
        'adjacencies' : []
      }
      # adding adjacencies to the node representation
      for adjacency in data['adjacency'][i]:
        adj_dict = {
          'nodeTo' : unicode(adjacency['id']),
          'nodeFrom' : unicode(node['id']),
          'data' : {
            'weight' : unicode(adjacency['weight']),
            'label' : unicode(adjacency['label'])
          }
        }
        node_dict['adjacencies'].append(adj_dict)
      infovis.append(node_dict)
      i += 1
    # returning the resulting JSON string
    return json.dumps(infovis)

  def _get_nx_graph(self,res_type='stmt',label=True,weight=True):
    # returns a NetworkX representation of the selected graph (statement or
    # provenance)

    G = nx.Graph()
    if res_type == 'stmt':
      for s,p,o,w in [self.index.suid2stmt[x] for x in self.suid_dict.keys()]:
        # setting the nodes first
        if not s in G:
          G.add_node(s)
          if label:
            G.node[s]['label'] = self.index.lexicon[s]
          if weight: # relevance of the node
            G.node[s]['weight'] = self.tuid_set[s]
        if not o in G:
          G.add_node(o)
          if label:
            G.node[o]['label'] = self.index.lexicon[o]
          if weight: # relevance of the node
            G.node[o]['weight'] = self.tuid_set[o]
        # setting the edge now
        G.add_edge(s,o)
        if label:
          G[s][o]['weight'] = w
          G[s][o]['label'] = self.index.lexicon[p]
    elif res_type == 'prov':
      # @TODO - implement this
      pass
    return G
