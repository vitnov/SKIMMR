...
class FuzzySet:
  ...
  def __init__(self,elements=[],normalise=False):
    ...
    if normalise:
      n = sum(self.members.values())
      for member in self.members:
        self.members[member] /= n
  ...
  def max_elem(self):
    # returns element with the maximum membersihp degree and the degree

    mx_m, mx_d = None, 0.0
    for member in self.members:
      degree = self.members[member]
      if degree > mx_d:
        mx_m, mx_d = member, degree
    return (mx_m, mx_d)


  def min_elem(self):
    # returns element with the minimum membersihp degree and the degree

    mn_m, mn_d = None, 1.0
    for member in self.members:
      degree = self.members[member]
      if degree < mn_d:
        mn_m, mn_d = member, degree
    return (mn_m, mn_d)
